...
export async function updateUserEmbedding(
  ...
  try {
    ...
    if (normalizedRating < LIKE_THRESHOLD) {
      console.log(`[DobbySense] Rating ${normalizedRating} is below threshold. usage vector unchanged.`);
      return;
    }
    ...
    const embeddingTable = itemType === "movie" ? "movie_embeddings" : "show_embeddings";
    const idCol = itemType === "movie" ? "movie_id" : "show_id";

    console.log(`[DobbySense] Fetching embedding for ${itemType} (TMDB: ${itemId})`);
     ...
     if (directError) {
       console.error(`[DobbySense] Error fetching embedding by TMDB ID:`, directError);
     }
     ...
     if (!itemVector) {
       ...
       if (itemError) {
         console.error(`[DobbySense] Error fetching ${itemType} ID:`, itemError);
         return;
       }

       if (!itemData) {
         console.warn(`[DobbySense] ${itemType} not found for TMDB ${itemId}`);
         return;
       }

       const internalId = itemData.id;
       console.log(`[DobbySense] Resolved to Internal ID: ${internalId}. Fetching vector...`);
       ...
       if (vectorError) {
         console.error(`[DobbySense] Error fetching embedding:`, vectorError);
         return;
       }
     ...
     if (!itemVector) {
       console.log(`[DobbySense] ${itemType} not in DobbySense database - skipping learning.`);
       return;
     }
    ...
    console.log(`[DobbySense] Vector updated. Magnitude of shift: ${Math.sqrt(totalShift).toFixed(6)}`);
    console.log(`[DobbySense] First 3 dims: [${userVector.slice(0,3).map(n=>n.toFixed(3))}] -> [${newVector.slice(0,3).map(n=>n.toFixed(3))}]`);
    ...
    console.log(`[DobbySense] Updated user ${userId} embedding based on ${itemType} ${itemId}`);
