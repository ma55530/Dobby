...
export function SignUpForm({ className, onSwitchToLogin, ...props }: SignUpFormProps) {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (selectedGenres.length < 3) {
      setError(`Please select at least 3 genres (selected: ${selectedGenres.length})`)
      return
    }
    ...
    try {
      ...
      if (data.user) {
        ...
        if (preferences.length > 0) {
          const { error: prefError } = await supabase.from("user_genre_preferences").insert(preferences)
          ...
          if (prefError) {
            ...
            await fetch("/api/dobbySenseAPI/fold-in", {
              ...
              body: JSON.stringify({ selectedGenres: selectedGenreKeys }),
            })
              .then(async (res) => {
                if (res.ok) {
                  console.log("Fold-in success, triggering recommendation engine...");
                  // After fold-in, run recommendation engine to generate recommendations
                  await fetch("/api/recommendation-engine?limit=20")
                    .then(r => {
                      if(!r.ok) console.error("Rec engine failed:", r.status, r.statusText)
                      else console.log("Rec engine success");
                    })
                    .catch((err) =>
                    console.error("Recommendation engine error:", err),
                  )
                } else {
                  console.error("Fold-in failed:", res.status, await res.text());
                }
              })
              .catch((err) => console.error("Fold-in fetch error:", err))
          }
        }
      } else {
        console.warn("User created but no session returned. Email confirmation might be required.");
      }
                ...
                <>
                  <div className="grid gap-4">
                    <div className="flex items-center justify-between mb-1">
                      <div>
                        <Label className="text-white text-lg font-bold">Pick Your Favorites</Label>
                        <p className="text-sm text-gray-400 mt-1">Select at least 3 genres to personalize your feed</p>
                      </div>
                    </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={cn(
                                ...
                                isSelected ? "bg-purple-400 shadow-[0_0_6px_rgba(192,132,252,0.8)]" : "bg-transparent"
                            )} />
                          </button>
                        )
                      })}
                    ...
                    >
                      {isLoading ? (
                        <span className="flex items-center gap-2">
                          ...
                          <span className="animate-pulse text-sm">{LOADING_MESSAGES[loadingMessageIndex]}</span>
                        </span>
                      ) : (
                        "Create Account"
                      )}
