...
export async function GET() {
  ...
  try {
    ...
    if (watchlistsError) {
      ...
      return NextResponse.json({ error: 'Failed to fetch watchlists' }, { status: 400 })
    }
    ...
    const watchlistsWithItems = await Promise.all(
      watchlists.map(async (watchlist) => {
        ...
        if (itemsError) {
          console.error(`Error fetching items for watchlist ${watchlist.id}:`, itemsError)
    ...
    if (watchlistName) {
      ...
      if (existingWatchlist) {
        ...
        if (createError) {
          ...
          return NextResponse.json({ error: 'Failed to create watchlist' }, { status: 400 })
        }
