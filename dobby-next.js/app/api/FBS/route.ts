...
function isBadMovie(movie: Movie | Movies, config: MovieFilterConfig = FILTER_MOVIE_CONFIG): boolean {
...
function isBadShow(show: Show | Shows, config: ShowFilterConfig = FILTER_SHOW_CONFIG): boolean {
...
async function findBetterSimilarMovie(id: number, request: Request, fetchOptions: RequestInit): Promise<Movie | null> {
    try {
...
async function findBetterSimilarShows(id: number, request: Request, fetchOptions: RequestInit): Promise<Show | null> {
    try {
...
async function fetchMovieDetail(id: number, request: Request, fetchOptions: RequestInit): Promise<Movie | null> {
    try {
        const res = await fetch(new URL(`/api/movies/${id}`, request.url).toString(), fetchOptions);
...
async function fetchShowDetail(id: number, request: Request, fetchOptions: RequestInit): Promise<Show | null> {
    try {
        const res = await fetch(new URL(`/api/shows/${id}`, request.url).toString(), fetchOptions);
...
export async function GET(request: Request) {
    try {
        ...
        const fetchOptions = { headers: { cookie, accept: 'application/json' } } as RequestInit;

        // Fetch top movies and shows for user via RPC
        const [{ data: movieRows, error: movieRpcError }, { data: showRows, error: showRpcError }] = await Promise.all([
        ...
        if (movieRpcError) return NextResponse.json({ error: movieRpcError.message }, { status: 500 });
        if (showRpcError) return NextResponse.json({ error: showRpcError.message }, { status: 500 });

        const movieIds = (movieRows || []).map((row: any) => row.movie_id ?? row.id).filter((id: number) => typeof id === 'number');
        const showIds = (showRows || []).map((row: any) => row.show_id ?? row.id).filter((id: number) => typeof id === 'number');

        // Fetch details
        const movieDetails = (await Promise.all(movieIds.map((id: number) => fetchMovieDetail(id, request, fetchOptions)))).filter(Boolean) as Movie[];
        const showDetails = (await Promise.all(showIds.map((id: number) => fetchShowDetail(id, request, fetchOptions)))).filter(Boolean) as Show[];

        // Replace bad movies/shows with better similars when possible
        const processedMovies = (await Promise.all(movieDetails.map(async (movie) => {
            while (isBadMovie(movie)) {
                const better = await findBetterSimilarMovie(movie.id, request, fetchOptions);
                if (!better) break;
                movie = better;
            }
            return movie;
        ...
        const processedShows = (await Promise.all(showDetails.map(async (show) => {
            while (isBadShow(show)) {
                const better = await findBetterSimilarShows(show.id, request, fetchOptions);
                if (!better) break;
                show = better;
            }
            return show;
        ...
        if (processedMovies.length > 0) {
            const moviePayload = processedMovies.map((m) => ({
                user_id: userId,
                movie_id: m.id
            }));
            await supabase.from('movie_recommendations').delete().eq('user_id', userId);
            await supabase.from('movie_recommendations').insert(moviePayload);
        }

        if (processedShows.length > 0) {
            const showPayload = processedShows.map((s) => ({
                user_id: userId,
                show_id: s.id
            }));
            await supabase.from('show_recommendations').delete().eq('user_id', userId);
